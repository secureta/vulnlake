...
セキュリティデータセット (EPSS / CVE / GHSA / ExploitDB) を frozen DuckLake として
...
uv run vlake update epss         # dataset は epss|cve|ghsa|exploitdb
...
索引メタデータのみ Parquet 化し `code_url` で参照する設計。データセット追加・変更時は
