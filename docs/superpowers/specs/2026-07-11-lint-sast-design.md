...
- **zizmor** — ワークフローの SAST。権限過剰、テンプレートインジェクション、信頼できない checkout 等を検査する。`uvx zizmor` で実行する
...
- `lint` ジョブのステップ:
  ...
  3. actionlint の実行(GitHub Release バイナリまたは公式アクションで取得)
  4. `uvx zizmor .github/workflows/`
