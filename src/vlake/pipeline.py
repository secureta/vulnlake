...
def verify(cfg: Config, max_age_days: int | None = None) -> dict:
    ...
    検出できるもの: キー集合のずれ (未登録ファイルの混入、登録済みファイルの
    消失、キーが変わる差し替え)。件数一致では見逃すため、テーブル別の登録パス
    集合をストレージの実キー集合と突き合わせる。統計 (count/min/max) は
    カタログのメタデータで解決されるため、リモートでも全 Parquet の読み込みは
    発生しない。

    検出できないもの: 同一キーのまま中身だけを差し替えられた場合。パス集合も
    統計もカタログ側の値しか見ないため、Parquet 本体の書き換えは素通りする。
    塞ぐにはファイル単位のハッシュ等をカタログに持たせて実体と照合する必要が
    あり、日次 verify の I/O コストとのトレードオフになる。
